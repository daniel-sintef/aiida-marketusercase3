...
def prepare_inputs(user_input):
    ...
    Precvel = (
        inputs["PreCursorVolumeFlowRate"]
        / 1000.0
        / 1000.0
        / 60.0
        / (math.pi / 4.0 * dnozzle**2)
    )
    Dispmfr = (
        mwO2
        * atm
        * (inputs["DispersionVolumeFlowRate"] / 1000.0 / 60.0)
        / rgas
        / Tref
        / 1000.0
    )
    myprec = math.exp(
        ATSBmf * math.log(myATSB)
        + Xylenemf * math.log(myXylene)
        - 11.24 * ATSBmf * Xylenemf
    )
    ...
    Pilotch4mfr = (
        mwMethane
        * atm
        * (inputs["PilotMethaneVolumeFlowRate"] / 1000.0 / 60.0)
        / rgas
        / Tref
        / 1000.0
    )
    PilotO2mfr = (
        mwO2
        * atm
        * (inputs["PilotOxygenVolumeFlowRate"] / 1000.0 / 60.0)
        / rgas
        / Tref
        / 1000.0
    )
    Dropsmd = (
        51.0 * dnozzle * Re ** (-0.39) * We ** (-0.18) * (Precmfr / Dispmfr) ** 0.29
    )
    Pilotmfr = Pilotch4mfr + PilotO2mfr
    Pilotch4mf = Pilotch4mfr / Pilotmfr
    Piloto2mf = 1.0 - Pilotch4mf
    Fanextrate = (
        inputs["FanExtractionVolumeFlowRate"]
        * (atm * mwAir / rgas / Tref / 1000.0)
        / 3600.0
    )
...
def write_journalfile(inputs, fileout):
    ...
    """
    out = journal_template.format(
        ATSBmf=inputs["ATSBMassFraction"],
