...
``aiida-marketusercase3`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-marketusercase3`` repository (preferably using ``aiida-marketusercase3`` as the project name)

The documentation is now available at `aiida-marketusercase3.readthedocs.io <http://aiida-marketusercase3.readthedocs.io/>`_.
